...
export default function Home() {
  ...
  // Shuffle client-side after hydration to avoid SSR/client mismatches
  useEffect(() => {
    setItems((prev) => {
      const shuffled = [...prev];
  ...
  // Hue (CSS filter; static per selected & per shape)
  const [hue, setHue] = useState(0);
  const currentColor = `hsl(${hue} 100% 50%)`;

  // Remember gallery scroll so we can restore it
  const scrollPosRef = useRef(0);

  // Cache of complementary background colors per SVG
  ...
  // Shapes state is only updated when shapes are added/removed.
  ...
  const addShapesFromSource = useCallback(
    (
      ...
      const newShapes: Shape[] = Array.from({ length: 3 }, () => {
        ...
        const scaleAmp = 0.32 + Math.random() * 0.14; // more drastic breathing
  ...
  // Spawn THREE shapes per selection; keep cap high
  const handleSelect = () => {
  ...
  // Ultra-smooth animation loop (no React setState per frame)
  useEffect(() => {
    if (view !== "main") return;
    let rafId: number;
    ...
    const tick = () => {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        s.animState = { x, y, dx, dy, angle, dAngle, scaleBase, scaleAmp, phase, dPhase, scale };
        s.animState.opacity = opacity;
        s.animState.opacityTarget = opacityTarget;
        s.animState.opacityStep = opacityStep;
  ...
  // Restore scroll when coming back to gallery
  useEffect(() => {
    if (view === "gallery") {
  ...
  // Compute complementary colors for loaded items
  useEffect(() => {
    if (typeof window === "undefined") return;
    let cancelled = false;
  ...
  // Ensure selected item has a computed background
  useEffect(() => {
